...
macro_rules ! wlog { ($ log : expr , $ ($ arg : tt) *) => { { let s = format ! ($ ($ arg) *) ; if !$ log . compact { println ! ("{}" , s) ; } $ log . buffer . push_str (& s) ; $ log . buffer . push ('\n') ; } } ; }
pub fn run_analysis(args: &RunArgs, compact: bool) -> bool {
    ...
    let target_file = match discovered_target_file {
        Some(f) => f,
        None => {
            eprintln!(
                "CovOpt Error: Could not auto-discover target file. Ensure tests execute the target and that it's compiled with debug info."
            );
            std::process::exit(1);
        }
    };
    ...
    if args.require_cache_padding {
        ...
        if applicable {
            ...
            static_cache_padding = Some(true);
    ...
    if args.require_watchdog_timeout {
        ...
        if applicable {
            ...
            static_watchdog_timeout = Some(true);
            wlog!(log, "Static Watchdog Timeout: Not Applicable (Pure Function / No Concurrency)");
        }
    }
    let mut static_stress_test = None;
    if args.require_stress_test {
        let (has_stress, applicable) = static_analysis::analyze_project_stress_test(std::path::Path::new(&target_file));
        ...
        if applicable {
            ...
            static_stress_test = Some(true);
            wlog!(log, "Static Stress Test: Not Applicable (Pure Function / No Concurrency)");
        }
    }
