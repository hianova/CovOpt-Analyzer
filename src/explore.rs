...
pub fn run(src_dir: &str, trait_name: &str, method_name: &str, threshold: f64) {
    println!("🔍 [Explore] Scanning codebase for `{}` implementations in `{}`...", trait_name, src_dir);
            ...
            for item in ast.items {
                if let Item::Impl(impl_item) = item {
                    if let Some((_, path, _)) = &impl_item.trait_ {
                        if path.segments.last().map(|s| s.ident.to_string()) == Some(trait_name.to_string()) {
                            ...
                            if found_method {
                                ...
                                let mod_path = rel_path.with_extension("").iter().map(|s| s.to_string_lossy().to_string()).collect::<Vec<_>>().join("::");
    ...
    println!("✅ Found {} valid {} objectives!", unique_obj.len(), trait_name);
    ...
    println!("✅ Projected {} PERFECT pairs (Similarity >= {})!", perfect_pairs.len(), threshold);
    ...
    println!("🚀 [Mega-Batch Engine] Compiling single Mega-Batch of {} pairs...", perfect_pairs.len());
    ...
    println!("⏳ Rust is now compiling the Mega-Batch Harness in {:?}. This may take a few minutes...", target_root);
    
    let start_time = std::time::Instant::now();
    let output = Command::new("cargo")
        .current_dir(target_root)
        .args(&["test", "--test", "covopt_explore_harness", "--release", "--", "--nocapture"])
        .output();
    ...
    match output {
        Ok(out) => {
            ...
            if !out.status.success() {
                println!("❌ Harness compilation or runtime failed! (Took {:.1?}s)", elapsed);
    ...
    if dir.is_dir() {
        if let Ok(entries) = fs::read_dir(dir) {
            for entry in entries.flatten() {
                ...
                } else if path.extension().map(|s| s == "rs").unwrap_or(false) && path.file_name().unwrap() != "mod.rs" {
                    files.push(path);
                }
            }
        }
...
fn cosine_similarity(counter1: &HashMap<String, usize>, counter2: &HashMap<String, usize>) -> f64 {
    ...
    for k in counter1.keys() { terms.insert(k.clone()); }
    for k in counter2.keys() { terms.insert(k.clone()); }

    let mut dotprod = 0.0;
    for k in &terms {
        dotprod += (*counter1.get(k).unwrap_or(&0) as f64) * (*counter2.get(k).unwrap_or(&0) as f64);
    }

    let mag_a = counter1.values().map(|&v| (v as f64).powi(2)).sum::<f64>().sqrt();
    let mag_b = counter2.values().map(|&v| (v as f64).powi(2)).sum::<f64>().sqrt();
...
fn generate_harness(pairs: &[(f64, Objective, Objective)]) -> String {
    ...
    for (_, a, b) in pairs {
        ...
        let inst_a = if a.has_new { format!("{}::new()", a.name) } else { a.name.clone() };
        let inst_b = if b.has_new { format!("{}::new()", b.name) } else { b.name.clone() };
        
        let name_a = a.name.replace("Objective", "").replace("Macro", "").replace("Micro", "");
        let name_b = b.name.replace("Objective", "").replace("Macro", "").replace("Micro", "");
