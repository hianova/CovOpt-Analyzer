...
impl DiscreteDiffusionEngine {
    ...
    fn parse_inst(id: usize, line: &str) -> InstMeta {
        ...
        let is_branch = line.contains(" b ") || line.contains("\tb") || line.contains("cbz") || line.contains("cbnz") || line.contains("ret");
        let sets_flags = line.contains("cmp") || line.contains("adds") || line.contains("subs") || line.contains("ands");
        ...
        if parts.len() > 1 {
            ...
            for (i, part) in parts.iter().skip(1).enumerate() {
                if (part.starts_with('x') || part.starts_with('w') || part.starts_with('v') || part.starts_with('q'))
                    && part.len() <= 3 && part[1..].parse::<u32>().is_ok()
                {
                    if i == 0 && !is_store {
        ...
        InstMeta { id, writes, reads, is_mem, is_branch }
    }
    ...
    fn is_valid_permutation(candidate: &[(usize, String)], edges: &HashSet<(usize, usize)>) -> bool {
        ...
        for step in 0..self.diffusion_steps {
            ...
            let mut scored_candidates: Vec<(f64, Vec<(usize, String)>)> = canvas
                ...
                .map(|candidate| {
                    ...
                    let asm_text = candidate.iter().map(|(_, s)| s.clone()).collect::<Vec<_>>().join("\n");
            ...
            if best_score != f64::MAX {
                println!("  [Optimizer] Step {}/{} - Best Objective Score: {:.4}", step + 1, self.diffusion_steps, best_score);
            } else {
                println!("  [Optimizer] Step {}/{} - No valid permutations found yet", step + 1, self.diffusion_steps);
            }
        }
