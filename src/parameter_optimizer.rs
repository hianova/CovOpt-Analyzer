...
impl ParameterOptimizer {
        ...
        for p in param_str.split(',') {
            ...
            if parts.len() == 2 {
                ...
                if range_parts.len() == 2 {
                    if let (Ok(min), Ok(max)) = (range_parts[0].parse::<f64>(), range_parts[1].parse::<f64>()) {
    ...
    pub fn run(&self) {
        ...
        let epochs = 5;
        let samples_per_epoch = if self.iterations < epochs { 1 } else { self.iterations / epochs };

        for epoch in 1..=epochs {
            println!("🔍 [Epoch {}/{}] Logarithmic Bound Refinement:", epoch, epochs);
            ...
            for i in 1..=samples_per_epoch {
                ...
                if score > best_score {
                    ...
                    println!("  [Iter {}/{}] 🎉 New Global Best Score: {:.4} | Params: {:?}", i, samples_per_epoch, best_score, best_params);
                } else {
                    println!("  [Iter {}/{}] Score: {:.4} | Params: {:?}", i, samples_per_epoch, score, current_params);
                }
            }
            ...
            for (name, bounds) in current_bounds.iter_mut() {
                if let Some(&best_val) = epoch_best_params.get(name) {
                    ...
                    if new_min < global_bounds.min { new_min = global_bounds.min; }
                    if new_max > global_bounds.max { new_max = global_bounds.max; }

                    if bounds.is_int {
                        new_min = new_min.floor();
                        new_max = new_max.ceil();
                        if new_min == new_max {
                            if new_min > global_bounds.min { new_min -= 1.0; }
                            if new_max < global_bounds.max { new_max += 1.0; }
                        }
                    }
        ...
        let _ = std::fs::write(".covopt_tuned.env", format!("# Auto-generated by CovOpt-Analyzer\n# Target Test: {}\n# Best Score: {:.4}\n{}", self.target_test, best_score, env_content));
