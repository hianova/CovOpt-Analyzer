...
pub fn analyze_memory_ops(asm_block: &str) -> MemoryProfile {
    ...
    for line in asm_block.lines() {
        ...
        if l.contains("call") || l.contains("bl ") {
            if l.contains("alloc") || l.contains("malloc") || l.contains("push") || l.contains("reserve") {
                profile.allocs += 1;
            }
        }
...
pub fn analyze_thread_activity(source_file: &Path) -> Vec<String> {
    ...
    let has_spawn = content.contains("thread::spawn") || content.contains("tokio::spawn") || content.contains("async_std::task::spawn");
